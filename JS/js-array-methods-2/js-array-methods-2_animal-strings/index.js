...
// Hint: You can pass the starting index as second parameter.
 const catStartingFromIndexFiveExists = !!animalStrings.find((animalString, index) => animalString==="cat" && index > 5);
...
const indexOfGiraffe = animalStrings.findIndex((animalString) => animalString === "giraffe");
...
const anyAnimalEndsWithLetterZ = animalStrings.find((animalString) => animalString.endsWith("z"));
console.log(anyAnimalEndsWithLetterZ);

const everyAnimalHasMoreThanTwoLetters = animalStrings.find((animalString) => animalString.length > 2);
...
const sumOfAllAnimalCharacters = animalStrings.map((animalString) => animalString.length).reduce((a, b) => a + b);
// const sum = sumOfAllAnimalCharacters.reduce((a, b) => a + b);
